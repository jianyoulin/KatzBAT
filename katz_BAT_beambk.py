# this code record number oof licks via bream break sensor
# sensory connect to GPIO 6 (D6); Pin 31
...
#     np.save('current_trial.npy', np.array(index, dtype=np.int))
    ...
    # setup current and destinate spout positions
    if index == 0:
        cur_pos = 1
...
#     print(f'{turn_dir=}, {cur_pos=}, {dest_pos=}')   
    ...
    # detecting the current status of bream break sensor
    ...
    while (time.time() - trial_init_time < cur_trial_time) and \
        ...
        if current_poke == 1 and last_poke == 0:
            ...
            if beam_unbroken - beam_break > 0.02: # to avoid noise (from motor)- induced licks
                licks[this_spout][lasers[index]][-1].append(beam_break)
                ordered_licks[f'{index}'].append(beam_break)

                if len(licks[this_spout][lasers[index]][-1]) == 1:
                    first_lick = beam_break
